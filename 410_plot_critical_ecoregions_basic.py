...
def plot_pca_clusters_with_map(
    ...
    # Load and filter combined effects
    ...
    #     alpha=0.7
    # )
    # Base plot: All ecoregions colored by cluster
    scatter = ax1.scatter(
        PC_scores[pc_x_col] * scale_x,
        PC_scores[pc_y_col] * scale_y,
        c=clusters,
    ...
    # Highlighted red points (same as map)
    ...
    for _, row in PC_scores.iterrows():
        ax1.text(
    ...
    # --- Right: Map plot ---
    gdf.boundary.plot(ax=ax2, color="lightgray", linewidth=0.2)
